// CACHE FIX: Wrapped in function that handles multiple load states
(function() {
    'use strict';
    ...
    function initializeTOC() {
        ...
        // Exit if already initialized (prevents double-initialization with caching)
        if (tocElement.dataset.initialized === 'true') {
            return;
        }
        tocElement.dataset.initialized = 'true';
    ...
    // CACHE FIX: Try multiple initialization methods to ensure it runs
    
    // Method 1: DOMContentLoaded (best for normal page loads)
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeTOC);
    } else {
        // Document already loaded (cached pages)
        initializeTOC();
    }
    
    // Method 2: Load event as backup (for heavily cached pages)
    window.addEventListener('load', function() {
        // Only run if not already initialized
        const tocElement = document.querySelector('.patoc-toc');
        if (tocElement && tocElement.dataset.initialized !== 'true') {
            initializeTOC();
        }
    });
